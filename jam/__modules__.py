...
from jwt.test_jwt_tools import public_key


class BaseModule:
    """The base module from which all other modules inherit."""

    def __init__(self, module_type: Literal[
        "jwt",
        "session-redis",
        "session-mongo",
        "session-custom"
        ...
        Args:
            module_type (Litetal["jwt", "session-redis", "session-mongo",
            "session-custom"]): Type of module
        """
...
class JWTModule(BaseModule):
    ...
    Methods:
        make_payload(exp: int | None, **data): Creating a generic payload for a
        token
    """
    ...
    def __init__(
        ...
        Args:
            alg (Literal["HS256", "HS384", "HS512", "RS256", "RS384",
            "RS512", "PS512", "PS384", "PS512"]): Algorithm for
            token encryption
    ...
    def make_payload(
        ...
        Args:
            ...
            **data: Custom data
        """
        _exp = lambda e=exp: e if e is not None else self.exp
        payload = {
            "jti": str(uuid4()),
            "exp": _exp,
